// Custom Zellij Generator - User-controlled parameters
// Based on Kaplan's line arrangement method with explicit control over all parameters
...
export interface CustomRenderOptions {
  ...
  // Custom controls (previously randomized)
  lineDensity: number;    // 4-25: controls grid complexity
  lineCount: number;      // 5-50: number of lines in the pattern
  focus: 'None' | 'Eight' | 'Sixteen';  // Focus pattern type
  
  // Visual options
  showOutlines: boolean;  // Whether to add stroke to shapes
...
// Random state (for filler selection and shimmer only)
let randSeed: number;
...
function createLines(num: number): { lines: Line[]; groups: Point[][] } {
  ...
  function makeStar(n: number): void {
    ...
    for (const step of plan) {
      if (step.idx >= 0 && step.idx < all_lines.length) {
        if (step.keep) {
  ...
  // 8-pointed star focus - uses deterministic center position
  function make2x2(n: number): void {
    const ax = Math.floor(n / 2);
    const ay = Math.floor(n / 2);
  ...
  if (FOCUS === 'Eight') {
    make2x2(LINE_DENSITY);
  } else if (FOCUS === 'Sixteen') {
    makeStar(LINE_DENSITY);
  }

  // Discount the lines already used
...
function applyShimmer(hex: string, shimmerLevel: number): string {
  ...
  const intensity = shimmerLevel * 0.15;
...
// Main custom render function
export function generateCustomZellij(options: CustomRenderOptions): CustomRenderedShape[] {
  const { 
    seed, 
    width, 
    height, 
  ...
  // Set explicit parameters (no randomization)
  LINE_DENSITY = Math.max(4, Math.min(25, lineDensity));
  ...
  const paddingValue = Math.max(20, Math.min(100, padding));
  const cbox = makeBox(xmin, ymin, xmax - xmin, ymax - ymin);
  const sbox = makeBox(paddingValue, paddingValue, width - paddingValue * 2, height - paddingValue * 2);
  const M = fillBox(cbox, sbox, false);

  const drawnShapes: CustomRenderedShape[] = [];

  // Draw tiles
  for (const t of tiles) {
    drawTileCustom(M, t.path, palette, drawnShapes, showOutlines, outlineColor, outlineWidth);
  }

  return drawnShapes;
}

function drawTileCustom(
  M: AffineMatrix, 
  t: Point[], 
  palette: string[], 
  output: CustomRenderedShape[],
  ...
  // Use seed-based rotation for consistency
...
export interface ZellijPreset {
  name: string;
  lineDensity: number;
  lineCount: number;
  focus: 'None' | 'Eight' | 'Sixteen';
}

export const ZELLIJ_PRESETS: ZellijPreset[] = [
  { name: 'Simple', lineDensity: 6, lineCount: 9, focus: 'None' },
  { name: 'Classic', lineDensity: 10, lineCount: 25, focus: 'None' },
  { name: 'Complex', lineDensity: 15, lineCount: 35, focus: 'None' },
  { name: 'Dense', lineDensity: 20, lineCount: 45, focus: 'None' },
